...
def multi_stream(ngraph_train: list, ngraph_val, nnode, logger, algo_names,
                 ngraph_test: list, nnode_test: list, graph, setting, batchsize=10):
    # print(ngraph_train)
    # train_datafp = 'Data/train_%s%s_%s' % (graph, ngraph_train, nnode)
    # val_datafp = 'Data/val_%s%s_%s' % (graph, ngraph_val, nnode)
    # test_datafp = ['Data/test_%s%s_%s' %
    #                (graph, ngraph_test[i], nnode_test[i]) for i in range(len(nnode_test))]
    ...
    if setting == '1BF+1BFS':
        ...
        train_datafp = ['Data/train_%s%s_%s' %
                        (graph, ngraph, nnode) for ngraph in ngraph_train]
        val_datafp = 'Data/val_%s%s_%s' % (graph, ngraph_val, nnode)
        test_datafp = ['Data/test_%s%s_%s' %
                       (graph, ngraph_test[i], nnode_test[i]) for i in range(len(nnode_test))]
